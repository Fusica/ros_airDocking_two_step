...
def far_away_flag_cb(data):
    global far_away_flag
    far_away_flag = data
...
def timecallback(event):
    ...
    global mav_vel_receive
    global control_name
    global mode, start, far_away_flag
    ...
    if 1:  # start == 1:
        ...
        ## pid_control
        print(f"far_away_flag.data is {far_away_flag}")

        if control_name == "pid":
            print(f"far_away_flag.data is {far_away_flag.data}")
            if far_away_flag.data == 1:
                print(f"pid output")
                # pdb.set_trace()
            omega = np.array(
                [mav_vel_receive.twist.angular.x, mav_vel_receive.twist.angular.y, mav_vel_receive.twist.angular.z]
            ...
            # print(f"nominal_pos is {nominal_pos}")
            action = modelPredict(pos, vel, r_now.reshape(3, 3), omega, nominal_pos, init_flag=far_away_flag.data)
...
rospy.init_node("control_velocity_pid2", anonymous=True)
px4_command_pub_ = rospy.Publisher("/pid_cmd2", AttitudeTarget, queue_size=1)
nominal_pos_enu_pub = rospy.Publisher("/child2/nominal_pos_enu_pid", Point, queue_size=1)
first_pos_pub = rospy.Publisher("/child2/randinit_pos_pid", PoseStamped, queue_size=1)
offboard_start_pub = rospy.Publisher("/child2/offboard_start", Int32, queue_size=1)
state_error_pub = rospy.Publisher("/child2/state_error", Float64MultiArray, queue_size=1)
# mode_sub_


state_pos_sub = rospy.Subscriber("/child2/state_pos", Point, state_pos_cb, queue_size=1)
nominal_position_sub = rospy.Subscriber("/nominal_pid_pos2", PoseStamped, nominal_position_cb, queue_size=1)
...
mav_vel_receive_sub = rospy.Subscriber(
    "/child2/mavros/local_position/velocity_body", TwistStamped, mav_vel_receive_cb, queue_size=1
)
mode_sub = rospy.Subscriber("/child2/mavros/state", State, mode_cb, queue_size=1)

# debug
far_away_flag_sub = rospy.Subscriber("/far_away_flag", Int32, far_away_flag_cb, queue_size=1)
