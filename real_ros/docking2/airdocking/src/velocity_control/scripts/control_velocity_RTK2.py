...
def nominal_position_cb(data):
    global nominal_pos
    #nominal_pos = np.array([0.2, 0., 2.0])
    
    
    nominal_pos = np.array([data.pose.position.y, -data.pose.position.x-1.0, data.pose.position.z])
...
def local_pos_cb(data):
    ...
    # data.pose.position.x = data.pose.position.x + fixed_vec.pose.position.x
    # data.pose.position.y = data.pose.position.y + fixed_vec.pose.position.y
    # data.pose.position.z = data.pose.position.z + fixed_vec.pose.position.z
...
def timecallback(event):
    ...
    if 1: #start == 1:
        ...
        #print(control_name)
        nominal_pos_enu_pub.publish(nominal_pos_enu) #mavros本身的值
        ...
        px4_command.thrust = (action[0] + 1) / 1.93 * 2 * 1.0 * 0.32 #0.50 #0.50
...
rospy.init_node('control_velocity_child2', anonymous=True)
px4_command_pub_ = rospy.Publisher("/rl_cmd2", AttitudeTarget, queue_size=1)
nominal_pos_enu_pub = rospy.Publisher("/child2/nominal_pos_enu", Point, queue_size=1)
first_pos_pub = rospy.Publisher("/child2/randinit_pos", PoseStamped, queue_size=1)
offboard_start_pub = rospy.Publisher("/child2/offboard_start", Int32, queue_size=1)
state_error_pub = rospy.Publisher("/child2/state_error_rl", Float64MultiArray, queue_size=1)
# mode_sub_


state_pos_sub = rospy.Subscriber("/child2/state_pos", Point, state_pos_cb, queue_size=1)
nominal_position_sub = rospy.Subscriber("virture_child1_pos", PoseStamped, nominal_position_cb, queue_size=100)
#nominal_eular_angles_sub = rospy.Subscriber("/nominal_euler_angles", Point, nominal_euler_angles_cb, queue_size=1)
arm_sub = rospy.Subscriber("/child2/start_pub_att", Bool, arm_cb, queue_size=1)
#get_rc_channel_sub = rospy.Subscriber("/mavros/rc/in", RCIn, get_rc_channel_cb, queue_size=1)

#rospy.Subscriber("/mavros/state", State, state_callback)
local_pos_sub = rospy.Subscriber("/child2/mavros/local_position/pose", PoseStamped, local_pos_cb, queue_size=1)
mav_vel_receive_sub = rospy.Subscriber("/child2/mavros/local_position/velocity_body", TwistStamped, mav_vel_receive_cb, queue_size=1)
mode_sub = rospy.Subscriber("/child2/mavros/state", State, mode_cb, queue_size=1)




rospy.Timer(rospy.Duration(0.01), timecallback)
# rospy.Timer(rospy.Duration(0.001), nominal_position_cb)
rospy.spin()