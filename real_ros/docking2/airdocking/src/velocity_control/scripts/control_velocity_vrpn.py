...
def nominal_position_cb(data):
    ...
    nominal_pos = np.array([data.pose.position.y, -data.pose.position.x - 1.1, data.pose.position.z])
...
def timecallback(event):
    ...
    if 1:  # start == 1:
        ...
        # 为什么这里需要这样定义呢？
        px4_command.thrust = (action[0] + 1) / 1.93 * 2 * 1.0 * 0.34  # 0.50 #0.50
...
nominal_position_sub = rospy.Subscriber(
    "/child1/mavros/local_position/pose", PoseStamped, nominal_position_cb, queue_size=1
)
