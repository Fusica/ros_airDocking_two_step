...
if __name__ == '__main__':
    ...
    # 子机二悬停检测
    # 第一个是在避障中使用，需要维护一个buffer，所以分开定义了
    ...
    while not rospy.is_shutdown():
        ...
        child12_expected_position_world_frame = pos_child1 + np.array([1.0, 0.0065,0.0241])
        ...
        if quadrotor2_mode == 'OFFBOARD':
            if quadrotor1_mode == 'OFFBOARD':
                child2_target_position_world_frame = np.array(
                        [
                            child2_pose.pose.position.x,
                            child12_expected_position_world_frame[1],
                            child12_expected_position_world_frame[2],
                        ]
                    )
                
                child1_postion_world_frame = np.array(
                    [child1_pose.pose.position.x, child1_pose.pose.position.y, child1_pose.pose.position.z]
                )
                ...
                yz_distance = np.linalg.norm(pos_child2[1:3] + np.array([0.0065, 0.0241]) - pos_child1[1:3])
                ...
                if far_away_flag:
                    ...
                    two_x = max(two_x, child1_pose.pose.position.x + 0.68)
                ...
                print('expected pos:{}'.format(next_child2_target_position_world_frame))
                #  print('expected pos:{}'.format(child12_expected_position_world_frame))

                send_flag = False

                RL_nominal_pos.pose.position.x = two_x
                RL_nominal_pos.pose.position.y = child12_expected_position_world_frame[1]
                RL_nominal_pos.pose.position.z = child12_expected_position_world_frame[2]
