...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        tong_child1 = pos_child1 + child1_rot@child1_p
        
        child1_axis_position_world_frame[0] = tong_child1[0]
        
        child2_target_jian_world_frmae = np.array([child1_axis_position_world_frame[0]+distance, child1_axis_position_world_frame[1], child1_axis_position_world_frame[2]]) # 捅间距
        ...
        child2_jian_position_world_frame = np.array([child2_pose.pose.position.x, child2_pose.pose.position.y, child2_pose.pose.position.z]) + child2_rot@child2_p

        tong_global_pos.position.x = child1_axis_position_world_frame[0]
        tong_global_pos.position.y = child1_axis_position_world_frame[1]
        tong_global_pos.position.z = child1_axis_position_world_frame[2]
                    
        child1_tong_pub.publish(tong_global_pos)
                    
        jian_global_pos.position.x = child2_jian_position_world_frame[0]
        jian_global_pos.position.y = child2_jian_position_world_frame[1]
        jian_global_pos.position.z = child2_jian_position_world_frame[2]
                    
        child2_jian_pub.publish(jian_global_pos)
        ...
        if quadrotor2_mode == 'OFFBOARD':
            if quadrotor1_mode != 'OFFBOARD':
                ...
                child2_hover_flag = child2_hover_check.check(tolerance_variance_pos=0.03) # 悬停检测 0.7 # TODO:重新测定
                ...
                yz_distance = np.linalg.norm(child2_jian_position_world_frame[1:3] - child1_axis_position_world_frame[1:3])
                print(f"yz_distance is {yz_distance}")
                tong_x = child1_axis_position_world_frame[0]
                ...
                if far_away_flag:
                    ...
                    #vel_x -= 0.01
                    two_x -= 0.1 /100
                    two_x = max(two_x, child1_pose.pose.position.x + 0.9)  # 数值修改
                    rospy.loginfo("close")
                ...
                child2_hover_flag = child2_hover_check2.check(tolerance_variance_pos=0.03)
