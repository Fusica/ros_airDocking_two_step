    ...
    while not rospy.is_shutdown():
        ...
        if quadrotor2_mode == 'OFFBOARD':
            if quadrotor1_mode != 'OFFBOARD':
                ...
                if not force_flag:
                    child1_postion_enu = np.array([child1_pose.pose.position.x, child1_pose.pose.position.y, child1_pose.pose.position.z])

                    vel_x = 0.05 # 1号无人机的速度 m/s
                
                    # child2_tartget_postion.position.x = child1_postion_enu[0]
                    child2_tartget_postion.position.y = child1_postion_enu[1]
                    child2_tartget_postion.position.z = child1_postion_enu[2]
                    child2_tartget_postion.yaw = 0
                
                
                    child2_tartget_postion.velocity.x = vel_x
                
                    child2_tartget_postion.type_mask = 1+16+32+64+128+256+512+1024+2048 # 发布位置控制指令
                    
                    child1_attitude_pub.publish(child1_cmd) # 使用自带的控制器控制1号无人机悬停
                    child2_tartget_postion_pub.publish(child2_tartget_postion)
                else:
                    # 力位混合控制
                    child1_attitude_pub.publish(child1_cmd)
                    child2_attitude_pub.publish(child2_cmd)
