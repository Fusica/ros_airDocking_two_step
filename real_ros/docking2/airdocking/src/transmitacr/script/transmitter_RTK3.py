...
if __name__ == '__main__':
    ...
    child1_pose_sub = rospy.Subscriber(quadrotor_name1 + '/mavros/local_position/pose',PoseStamped,callback=child1_pose_callback, queue_size=10)
    child2_pose_sub = rospy.Subscriber(quadrotor_name2 + '/mavros/local_position/pose',PoseStamped,callback=child2_pose_callback, queue_size=10)
    ...
    virture_child1_tartget_postion_pub = rospy.Publisher('virture_child1_pos', PoseStamped, queue_size=100)
    ...
    # 虚拟一号机位置以二号机为原点高1.5m位置上, x轴-1方向上
    
    current_position = np.array([child2_pose.pose.position.x,child2_pose.pose.position.y,child2_pose.pose.position.z])
    
    print('current position {}'.format(current_position))
    
    virture_child1_tartget_postion = PoseStamped()
    
    virture_child1_tartget_postion.pose.position.x = 0.0 + child2_pose.pose.position.x
    virture_child1_tartget_postion.pose.position.y = -1.0 + child2_pose.pose.position.y
    virture_child1_tartget_postion.pose.position.z = 1.0 + child2_pose.pose.position.z
    ...
    while not rospy.is_shutdown():
        ...
        # 发送虚拟一号机位置
        virture_child1_tartget_postion_pub.publish(virture_child1_tartget_postion)
        
        child1_pose.pose.position.x = virture_child1_tartget_postion.pose.position.x 
        child1_pose.pose.position.y = virture_child1_tartget_postion.pose.position.y
        child1_pose.pose.position.z = virture_child1_tartget_postion.pose.position.z
        ...
        if quadrotor2_mode == 'OFFBOARD':
            if quadrotor1_mode != 'OFFBOARD':
                ...
                child2_hover_flag = child2_hover_check.check(tolerance_variance_pos=2.5) # 悬停检测 0.7
                ...
                child2_hover_flag = child2_hover_check2.check(tolerance_variance_pos=4.0)
